...
class Relatorios:
    @staticmethod
    def make_paths(population):
        try:
            if not os.path.exists('REPORT/'):
                os.mkdir('REPORT/')
            simulate_path = datetime.datetime.now().strftime("%d_%m_%Y")
            if os.path.exists(f'REPORT/{simulate_path}'):
                os.mkdir(f'REPORT/Relatório_{simulate_path}')
            memoria = open(f'REPORT/Relatório_{simulate_path}/{simulate_path}.json', 'w')
            for person in population:
                memoria.write("\n")
                memoria.write(person.name)
                memoria.write(person.sex)
                memoria.write(str(person.personality))
                memoria.write(str(person.birth_date))
                memoria.write(str(person.age))
                memoria.write(person.marital_status)
                memoria.write(person.partner)
                if person.status_life == 'Morte':
                    memoria.write(str(person.death_date))
                    memoria.write(str(person.age_death))
                    memoria.write(person.cause_death)
